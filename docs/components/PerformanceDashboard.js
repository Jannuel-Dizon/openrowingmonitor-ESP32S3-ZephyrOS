...
export class PerformanceDashboard extends AppElement {
  ...
  render() {
    ...
    return html`
      <dashboard-metric
        .icon=${icon_route}
        .unit=${metrics?.distanceTotal?.unit || "m"}
        .value=${metrics?.distanceTotal?.value}
      ></dashboard-metric>
      <dashboard-metric
        .icon=${icon_stopwatch}
        unit="/500m"
        .value=${metrics?.splitFormatted?.value}
      ></dashboard-metric>
      <dashboard-metric
        .icon=${icon_bolt}
        unit="watt"
        .value=${metrics?.power?.value}
      ></dashboard-metric>
      <dashboard-metric
        .icon=${icon_paddle}
        unit="/min"
        .value=${metrics?.strokesPerMinute?.value}
      ></dashboard-metric>
      ${metrics?.heartrate?.value
        ? html` <dashboard-metric
            .icon=${icon_heartbeat}
            unit="bpm"
        ...
        : html`<dashboard-metric
            .icon=${icon_paddle}
            unit="total"
            .value=${metrics?.strokesTotal?.value}
          ></dashboard-metric>`}
      <dashboard-metric
        .icon=${icon_fire}
        unit="kcal"
        .value=${metrics?.caloriesTotal?.value}
      ></dashboard-metric>
      <dashboard-metric
        .icon=${icon_clock}
        .value=${metrics?.durationTotalFormatted?.value}
      ></dashboard-metric>
  ...
  calculateFormattedMetrics(metrics) {
    ...
    for (const [key, value] of Object.entries(metrics)) {
      const valueFormatted = fieldFormatter[key]
        ? fieldFormatter[key](value)
        : value;
      if (
        valueFormatted.value !== undefined &&
        valueFormatted.unit !== undefined
      ) {
