...
export class DashboardActions extends AppElement {
  ...
  render() {
    return html`
      ...
      <button
        @click=${this.connectBluetooth}
        class=${this.appState.connected ? "connected" : ""}
      >
        ${icon_bluetooth}
      </button>

      <div class="peripheral-mode">
        ${this.appState.connected ? "Connected" : "Click to Connect"}
      </div>
  ...
  renderOptionalButtons() {
    ...
    if (
      this.appState?.appMode === "KIOSK" &&
      this.appState?.config?.shutdownEnabled
    ) {
      buttons.push(html`
